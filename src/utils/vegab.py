...
class CheckIfFinished(VegabPlugin):
    def on_parsed_args(self, args):
        if os.path.exists(os.path.join(args.save_path, "FINISHED")):
            logger.info("Finished")
            exit(0)
        return args

class SyncSmallFiles(VegabPlugin):
    """
    Saves useful meta information (including source code)
    """

    def __init__(self, user, host, root_folder_src, root_folder_dst):
        self.user = user
        self.host = host
        self.root_folder_src = root_folder_src
        self.root_folder_dst = root_folder_dst

    def on_after_call(self, config, save_path):
        if save_path[-1] == "/": # A bit of standarization
            save_path = save_path[0:-1]
        if self.root_folder_dst[-1] == "/": # A bit of standarization
            self.root_folder_dst = self.root_folder_dst[0:-1]
        if self.root_folder_src[-1] == "/": # A bit of standarization
            self.root_folder_src = self.root_folder_src[0:-1]

        if not save_path.startswith(self.root_folder_src):
            logger.warning("Skipping syncing due to nonstandard save_path")
            return

        add_folder = save_path[len(self.root_folder_src) + 1:]
        remote_save_dir = os.path.join(self.root_folder_dst, add_folder)

        # TODO: This can be much improved
        cmd = "ssh {user}@{host} 'mkdir -p {remote_save_dir}'".format(user=self.user, host=self.host,
        ...
        cmd = "rsync -a -v -r --max-size=1M {save_path} {user}@{host}:{remote_save_dir}".format(
            user=self.user, host=self.host, save_path=save_path, remote_save_dir=self.root_folder_dst)
        logger.info(cmd)
        ret1 = os.system(cmd)
...
def _parse_args(config_registry):
    ...
    for arg in sys.argv[1:]:
        if "=" in arg and "." in arg.split("=")[0]:
            ...
            logger.info("Interpreting " + arg + " as an entry for " + argname)
        else:
