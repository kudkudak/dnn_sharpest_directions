...
commonparams="--no_lanczos_inference_mode --lanczos_top_K=$K --lanczos_top_K_N_sample=$N --lanczos_kwargs=\"{'impl':'scipy'}\" --lanczos_aug ${commonparams}"

setparams=""

# Run no mom, no L2, CIFAR10
for config in vgg11_no_dropout; do #
for seed in 777; do
...
for lr in 0.01; do
    ...
    if grep 'Finished train' $save_path/stderr.txt 2> /dev/null; then
        ...
        echo "python bin/train_vgg_cifar.py $config $save_path  ${commonparams} --data_seed=${seed} --seed=$seed  --lr=$lr --opt_kwargs=\"{'overshoot':${overshoot},'KK':$KK}\"" >> $0.batch
    fi
done
done
done
