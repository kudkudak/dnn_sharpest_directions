...
EXPNAME=bn_ng_resnet
runsh_path=$0.batch
rm $0.batch

## 2. Create jobs
config=cifar10_resnet32
...
defaultparams="--lanczos_top_K=$K --lanczos_top_K_N_sample=$N --lanczos_kwargs=\"{'impl':'tf'}\" --lanczos_aug ${defaultparams}"
...
for lr in 0.1 0.01 0.001; do
    ...
    if grep 'Finished train' $save_path/stderr.txt 2> /dev/null; then
        ...
        if grep 'Corrupted' $save_path/run.sh.out 2> /dev/null; then
            echo "Reset"
            rm $save_path/*
        fi
        echo "python bin/train_resnet_cifar.py $config $save_path ${defaultparams} --lr=$lr" >> $runsh_path
    fi
done
...
for bs in $bss; do
    ...
    if grep 'Finished train' $save_path/stderr.txt 2> /dev/null; then
        ...
        echo "python bin/train_resnet_cifar.py $config $save_path ${defaultparams} --lr=${lr} --batch_size=${bs}" >> $runsh_path
    fi
done
...
for lr in 0.1 0.01 0.001; do
    ...
    if grep 'Finished train' $save_path/stderr.txt 2> /dev/null; then
        ...
        echo "python bin/train_resnet_cifar.py $config $save_path ${defaultparams} --lr=$lr  --measure_train_loss --epoch_size=${epoch_size} --n_epochs=${n_epochs} --reload" >> $runsh_path
    fi
done
...
for bs in $bss; do
    ...
    if grep 'Finished train' $save_path/stderr.txt 2> /dev/null; then
        ...
        echo "python bin/train_resnet_cifar.py $config $save_path ${defaultparams} --lr=$lr  --measure_train_loss --epoch_size=${epoch_size} --batch_size=${bs} --n_epochs=${n_epochs}" >> $runsh_path
    fi
done
