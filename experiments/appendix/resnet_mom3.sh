...
for lr in 0.01; do
    ...
    if grep 'Finished train' $save_path/stderr.txt 2> /dev/null; then
        ...
        echo "python bin/train_resnet_cifar.py $config $save_path  ${commonparams} --data_seed=${seed} --seed=$seed  --lr=$lr --opt_kwargs=\"{'type': 'correct_both', 'overshoot':${overshoot},'KK':$KK}\"" >> $0.batch
    fi
done
done
done
